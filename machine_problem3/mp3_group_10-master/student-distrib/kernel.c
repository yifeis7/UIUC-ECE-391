...
void entry(unsigned long magic, unsigned long addr)
{
    ...
    /* Clear the screen. */
    clear();
    // printf("Please let us pass ECE391!\n");
    ...
    //     : "memory"
    // );
    // uint32_t user_ds, user_esp, user_cs, user_eip, user_eflag;
    // user_ds = 0x002B;                         // 43
    // user_cs = 0x0023;                         // 35
    // user_esp = _128M + _4M - sizeof(uint32_t); // 0x83ffffc
    // user_eip = 0x80482e8;                     // 0x80482e8
    // asm volatile (
    //     "pushl %0           \n\t"    // user DS
    //     "pushl %1           \n\t"    // user ESP
    //     "pushfl             \n\t"    
    //     "popl %%eax         \n\t"
    //     "orl $0x200, %%eax  \n\t"
    //     "pushl %%eax        \n\t"    // user eflags (turn on IF)
    //     "pushl %2           \n\t"    // user CS
    //     "pushl %3           \n\t"    // user EIP
    //     "iret               \n\t"
    //     : 
    //     : "r" (user_ds), "r" (user_esp), "r" (user_cs), "r" (user_eip)
    //     : "memory", "eax", "ebx"
    // );
    // printf("Start ece391_execute!\n");
    // ece391_execute((uint8_t*)"shell");
    // printf("End ece391_execute, system ended.\n");
